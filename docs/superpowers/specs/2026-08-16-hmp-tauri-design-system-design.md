...
- 使用颜色层级、必要的边框和少量阴影表达结构关系，不给所有容器添加边框。
- 浅色和深色主题使用相同的语义令牌名称。
...
保留 `--walnut-50` 至 `--walnut-950` 色阶，但将色相整体收敛到偏红棕方向。浅色主题使用灰暖应用背景和浅色内容卡片；深色主题使用深红棕背景与深色面板，并以提亮后的红棕色作为主操作色。
...
- `--background: #D9D6D5`，使用偏灰的暖灰色作为应用底色。
- `--surface-1: #ECEAE9`，作为主要 content 区域。
- `--surface-2: #FAF9F8`，作为侧栏、播放器栏和卡片。
- `--surface-3: #FFFFFF`，作为浮层和更高层级卡片。
- `--primary: #934A3B`，主色使用中等饱和度红棕色。
- 边框使用低饱和暖灰色，不使用透明度。

### 4.2 深色主题

- `--background: var(--walnut-50)`，使用深红棕应用底色。
- `--surface-1: #1F1B17`，作为主要 content 区域。
- `--surface-2: #302321`，作为侧栏、播放器栏和卡片。
- `--surface-3: #44302D`，作为浮层和更高层级面板。
- 主色使用提亮后的红棕色 `--walnut-500`。
- 主要文本使用接近白色的 `--walnut-950`，次级文本继续使用中性色体系。
...
- 状态区域使用 `--surface-3` 或 `--surface-2`，不添加常规容器边框。
- 内部间距使用 `--space-2`。
...
- 禁用：使用弱化文本和边框颜色，不通过透明背景隐藏控件；边框只用于输入、焦点或确有分隔需求的控件。
